...
#if defined FREEBSD && defined _VGL && (__FreeBSD_version < 400000)
VGLBitmap      *
...
setpal(palette * pal)
{
  ...
  for (i = 0; i < 16; i++) {
		VGLSetPaletteIndex(i, (pal[i])[2], (pal[i])[1], (pal[i])[0]);
  }
}
...
vgaputim(Sint4 x, Sint4 y, Sint4 ch, Sint4 w, Sint4 h)
{
  ...
  for (i = 0; i < realsize; i++)
		if (tmp->Bitmap[i] != 0xff)
			scr->Bitmap[i] = (scr->Bitmap[i] & mask->Bitmap[i]) | tmp->Bitmap[i];
