...
void
drawdig(int n, int d, int x, int y, bool f)
{
...
bool
checkdiggerunderbag(Sint4 h,Sint4 v)
{
  int n;
  for (n=curplayer;n<diggers+curplayer;n++)
    if (digdat[n].alive)
...
void
killdigger(int n,Sint4 stage,Sint4 bag)
{
  if (digdat[n].invin)
    return;
...
bool
hitemerald(Sint4 x,Sint4 y,Sint4 rx,Sint4 ry,Sint4 dir)
{
  bool hit=FALSE;
  Sint4 r;
  if (dir!=DIR_RIGHT && dir!=DIR_UP && dir!=DIR_LEFT && dir!=DIR_DOWN)
